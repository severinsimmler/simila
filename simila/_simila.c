...
#define METHODS_ITEM(x)                \
  {                                    \
#x, x##_py, METH_VARARGS, x##_DESC \
  }
                   ...
                   size_t *lensum)
{
  ...
  if (PyObject_TypeCheck(arg1, &PyString_Type) && PyObject_TypeCheck(arg2, &PyString_Type))
  {
  ...
  else if (PyObject_TypeCheck(arg1, &PyUnicode_Type) && PyObject_TypeCheck(arg2, &PyUnicode_Type))
  {
...
PY_MOD_INIT_FUNC_DEF(simila._simila)
{
  PyObject *module;
  size_t i;

  PY_INIT_MOD(module, "simila._simila", Levenshtein_DESC, methods)
  /* create intern strings for edit operation names */
  if (opcode_names[0].pystring)
    abort();
  for (i = 0; i < N_OPCODE_NAMES; i++)
  {
    opcode_names[i].pystring = PyString_InternFromString(opcode_names[i].cstring);
                      ...
                      const lev_byte *strings2[])
{
  ...
  /* strip common prefix */
  while (n1 > 0 && n2 > 0 && *lengths1 == *lengths2 && memcmp(*strings1, *strings2, *lengths1 * sizeof(lev_byte)) == 0)
  {
    n1--;
    n2--;
    strings1++;
  ...
  /* strip common suffix */
  while (n1 > 0 && n2 > 0 && lengths1[n1 - 1] == lengths2[n2 - 1] && memcmp(strings1[n1 - 1], strings2[n2 - 1], lengths1[n1 - 1] * sizeof(lev_byte)) == 0)
  {
    n1--;
    n2--;
  }
                        ...
                        const lev_wchar *strings2[])
{
  ...
  /* strip common prefix */
  while (n1 > 0 && n2 > 0 && *lengths1 == *lengths2 && memcmp(*strings1, *strings2, *lengths1 * sizeof(lev_wchar)) == 0)
  {
    n1--;
    n2--;
    strings1++;
  ...
  /* strip common suffix */
  while (n1 > 0 && n2 > 0 && lengths1[n1 - 1] == lengths2[n2 - 1] && memcmp(strings1[n1 - 1], strings2[n2 - 1], lengths1[n1 - 1] * sizeof(lev_wchar)) == 0)
  {
    n1--;
    n2--;
  }
                         ...
                         size_t nb, const LevOpCode *bops)
{
  ...
  /* check completenes */
  if (bops->sbeg || bops->dbeg || bops[nb - 1].send != len1 || bops[nb - 1].dend != len2)
                     ...
                     size_t *nrem)
{
  ...
  *nrem = -1;
  ...
  for (i = 0; i < ns; i++)
  {
    while ((ops[j].spos != sub[i].spos || ops[j].dpos != sub[i].dpos || ops[j].type != sub[i].type) && j < n)
    {
